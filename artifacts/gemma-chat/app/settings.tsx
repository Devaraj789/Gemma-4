...
import { getEngineInfo } from "@/lib/inference";
...
export default function SettingsScreen() {
  ...
  const handleSaveApiKey = async () => {
    ...
    if (!trimmed) {
      // Clear key
      updateSettings({ openRouterApiKey: "", useOnlineModel: false });
      setKeyStatus("idle");
      return;
    }
    setValidating(true);
    ...
    try {
      ...
      if (result.valid) {
        ...
        Alert.alert(
          "Invalid API Key",
          result.error ?? "Key validation failed. Check your OpenRouter API key.",
        );
      }
    } catch {
      setKeyStatus("invalid");
      Alert.alert("Validation Error", "Could not reach OpenRouter. Check your internet connection.");
    } finally {
      setValidating(false);
    }
  };
  ...
  // Key status color helper
  const keyStatusColor =
    keyStatus === "valid" ? "#22c55e" :
    keyStatus === "invalid" ? colors.destructive :
    colors.mutedForeground;

  const keyStatusText =
    keyStatus === "valid" ? "✓ Key verified" :
    keyStatus === "invalid" ? "✗ Invalid key" :
    settings.openRouterApiKey ? "Key saved (not verified)" : "No key saved";
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
          ...
          <View style={[styles.profileCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.appSub, { color: colors.mutedForeground }]}>
                {getEngineInfo(settings)}
              </Text>
            </View>
            {/* Badge: Offline or Online based on current mode */}
            <View style={[
              styles.offlineBadge,
              { backgroundColor: settings.useOnlineModel ? "#3b82f620" : colors.success + "20" },
            ]}>
              <Feather
                name={settings.useOnlineModel ? "wifi" : "wifi-off"}
                size={12}
                color={settings.useOnlineModel ? "#3b82f6" : colors.success}
              />
              <Text style={[
                styles.offlineBadgeText,
                { color: settings.useOnlineModel ? "#3b82f6" : colors.success },
              ]}>
                {settings.useOnlineModel ? "Online" : "Offline"}
              </Text>
            </View>
          </View>
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={styles.infoRow}>
              ...
              <Text style={[styles.rowValue, { color: colors.mutedForeground }]} numberOfLines={1}>
                {settings.useOnlineModel ? "OpenRouter" : "llama.rn"}
              </Text>
            </View>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>ONLINE AI</Text>
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>

            {/* Toggle: Use Online Model */}
            <View style={styles.settingRow}>
              <View style={[styles.rowIconWrap, { backgroundColor: "#3b82f620" }]}>
                <Feather name="cloud" size={16} color="#3b82f6" />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[styles.rowLabel, { color: colors.foreground }]}>Use Online Model</Text>
                <Text style={[styles.rowSub, { color: colors.mutedForeground }]}>
                  {settings.openRouterApiKey ? "Switch between offline ↔ online" : "Set API key first"}
                </Text>
              </View>
              <Switch
                value={settings.useOnlineModel}
                onValueChange={(val) => {
                  if (val && !settings.openRouterApiKey.trim()) {
                    Alert.alert(
                      "API Key Required",
                      "Enter your OpenRouter API key below to use online models.",
                    );
                    return;
                  }
                  updateSettings({ useOnlineModel: val });
                }}
            ...
            <View style={{ paddingHorizontal: 16, paddingVertical: 12, gap: 8 }}>
              <Text style={[styles.rowLabel, { color: colors.foreground }]}>OpenRouter API Key</Text>
              <Text style={[styles.rowSub, { color: colors.mutedForeground }]}>
                Free at openrouter.ai — free tier models available
              </Text>

              {/* Input Row */}
              <View style={[
                apiKeyInputStyles.inputWrap,
                { backgroundColor: colors.secondary, borderColor: colors.border },
              ]}>
                <Feather name="key" size={15} color={colors.mutedForeground} style={{ marginLeft: 12 }} />
                <TextInput
                  value={apiKeyInput}
                  onChangeText={(t) => {
                    setApiKeyInput(t);
                  ...
                  autoCapitalize="none"
                  autoCorrect={false}
                  style={[apiKeyInputStyles.input, { color: colors.foreground }]}
                />
                {/* Show/Hide toggle */}
                <Pressable
              ...
              <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                <Text style={[styles.rowSub, { color: keyStatusColor }]}>
                  {keyStatusText}
                </Text>
                <Pressable
                  ...
                  disabled={validating}
                  style={({ pressed }) => [
                    apiKeyInputStyles.saveBtn,
                    { backgroundColor: colors.primary, opacity: pressed || validating ? 0.7 : 1 },
                  ]}
                >
                  {validating ? (
                    <ActivityIndicator size="small" color="#fff" />
                  ) : (
                    <Text style={apiKeyInputStyles.saveBtnText}>
                      {apiKeyInput.trim() ? "Save & Verify" : "Clear"}
                    </Text>
                  )}
                </Pressable>
              </View>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.rowLabel, { color: colors.foreground }]}>Get Free API Key</Text>
                <Text style={[styles.rowSub, { color: colors.mutedForeground }]}>openrouter.ai/keys</Text>
              </View>
            ...
            <Pressable
              onPress={() => router.push("/online-models")}
              style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.rowLabel, { color: colors.foreground }]}>Browse Online Models</Text>
                <Text style={[styles.rowSub, { color: colors.mutedForeground }]}>
                  {settings.openRouterModel || "No model selected"}
                </Text>
              </View>
              <Feather name="chevron-right" size={16} color={colors.mutedForeground} />
            </Pressable>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={{ paddingVertical: 4, paddingHorizontal: 0 }}>
              <Text style={[styles.sectionLabel, { color: colors.mutedForeground, marginBottom: 6, marginLeft: 16 }]}>CUSTOM SYSTEM PROMPT</Text>
              <TextInput
                ...
                placeholderTextColor={colors.mutedForeground}
                style={[styles.systemInput, { color: colors.foreground, backgroundColor: colors.secondary }]}
              />
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Pressable onPress={() => router.push("/saved")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
            ...
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={() => router.push("/language")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
              ...
              <Text style={[styles.rowValue, { color: colors.mutedForeground, marginRight: 6 }]}>{settings.language}</Text>
              <Feather name="chevron-right" size={16} color={colors.mutedForeground} />
            </Pressable>
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={() => router.push("/privacy")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
            ...
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={() => router.push("/network-check")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
            ...
            <View style={{ paddingHorizontal: 0 }}>
              ...
              <Slider label="" value={settings.autoDeleteDays} min={0} max={90} step={7} format={(v) => (v === 0 ? "Never" : `${v} days`)} onChange={(v) => updateSettings({ autoDeleteDays: v })} />
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Pressable onPress={() => Alert.alert("Rate App", "Thank you for using Gemma Offline Chat!")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
              <View style={[styles.rowIconWrap, { backgroundColor: "#f59e0b20" }]}>
            ...
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={() => void Linking.openURL("https://example.com/terms")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
              <View style={[styles.rowIconWrap, { backgroundColor: colors.primary + "15" }]}>
            ...
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={() => void Linking.openURL("https://example.com/privacy")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
              <View style={[styles.rowIconWrap, { backgroundColor: "#22c55e20" }]}>
            ...
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={() => router.push("/feedback")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Pressable onPress={() => router.push("/stats")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
              <View style={[styles.rowIconWrap, { backgroundColor: colors.primary + "15" }]}>
            ...
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={() => void handleExport()} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
              <View style={[styles.rowIconWrap, { backgroundColor: "#6366f120" }]}>
            ...
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={() => router.push("/storage")} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
              <View style={[styles.rowIconWrap, { backgroundColor: "#06b6d420" }]}>
            ...
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={handleClearChats} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
            ...
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Pressable onPress={handleReset} style={({ pressed }) => [styles.navRow, { opacity: pressed ? 0.7 : 1 }]}>
...
const apiKeyInputStyles = StyleSheet.create({
  inputWrap: {
    flexDirection: "row",
    alignItems: "center",
    borderRadius: 12,
...
const styles = StyleSheet.create({
  ...
  appIconCircle: { width: 52, height: 52, borderRadius: 14, alignItems: "center", justifyContent: "center" },
  appName: { fontSize: 15, fontFamily: "Inter_700Bold", marginBottom: 2 },
  appSub: { fontSize: 12.5, fontFamily: "Inter_400Regular" },
  offlineBadge: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 20 },
  offlineBadgeText: { fontSize: 11.5, fontFamily: "Inter_600SemiBold" },
  infoRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 14, gap: 12 },
  navRow: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 13, gap: 12 },
  ...
  settingRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 12, gap: 12 },
  segmentGroup: { flexDirection: "row", borderRadius: 10, padding: 3, gap: 2 },
  ...
  systemInput: { fontSize: 14, lineHeight: 20, fontFamily: "Inter_400Regular", padding: 12, borderRadius: 12, textAlignVertical: "top", minHeight: 90, marginHorizontal: 16, marginBottom: 12 },
  versionText: { textAlign: "center", fontSize: 12, fontFamily: "Inter_400Regular", paddingTop: 24, paddingBottom: 8 },
});